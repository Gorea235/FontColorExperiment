...
let currentDisplay: number = 0; // the current type of the display, set by the view when loaded
const _dispId: string = "#experimentDisplay"; // the id of the element storing the display
const _btnContinueId: string = "#btnContinue"; // the id of the continue button
const _questionGroup: string = "#questionGroup";

// displays an alert informing the user that the connection failed
function failedUpdate(_: any, textStatus: string) {
...
function checkStepFunc(success: () => void) {
    return (data: any) => {
...
function getQuestionAnswers() {
    var answers: any = {}; // {"question id": answer} answer: int, int[], string
...
function clickQuestion() {
    ...
    if (answers) {
        startPostData("/api/experiment/user/answer", checkStepFunc(nextStep), failedUpdate,
            { "id": userId(), "answers": JSON.stringify(answers) });
    }
    else {
...
function beginReadyPage() {
    if (userId()) {
        startPostData("/api/experiment/view", setupPage, (_, textStatus) => {
